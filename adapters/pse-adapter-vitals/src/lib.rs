...
impl VitalsAdapter {
    ...
    pub fn new(patient_id: &str) -> Self {
        Self {
            source: format!("vitals:{}", patient_id),
        }
    }
}
...
pub fn generate_embedded_data(seed: u64, duration_sec: u32) -> Vec<VitalReading> {
    ...
    for sample in 0..total_samples {
        ...
        let ecg_a =
            (t * hr_a / 60.0 * std::f64::consts::TAU).sin() * 1.2 + next_rng(&mut rng) * 0.05;
...
mod tests {
    ...
    #[test]
    fn test_vital_reading_roundtrip() {
        let r = VitalReading {
            patient_id: "A".into(),
            signal_type: VitalSignal::ECG,
            value: 1.2,
    ...
    fn test_pattern_roundtrip() {
        let p = VitalPattern {
            pattern_type: VitalPatternType::AtrialFibrillation,
            patient_id: "B".into(),
            confidence: 0.88,
            clinical_note: "test".into(),
        };
    ...
    fn test_ground_truth_window_is_patient_b_afib_region() {
        ...
        let expected_start = 2
            * ((AFIB_ONSET_FRACTION * duration_sec as f64 * EMBEDDED_SAMPLE_RATE_HZ as f64)
                as usize)
            + 1;
