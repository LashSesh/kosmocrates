...
pub fn hit_at_k(ranked: &[String], graded: &HashMap<String, u8>, k: usize) -> f64 {
    ...
    let hit = top.iter().any(|id| graded.get(*id).copied().unwrap_or(0) >= 1);
    if hit { 1.0 } else { 0.0 }
}
...
pub fn ndcg_at_k(ranked: &[String], graded: &HashMap<String, u8>, k: usize) -> f64 {
    ...
    let ideal_ids: Vec<String> = ideal_grades.iter().enumerate().map(|(i, _)| format!("ideal_{i}")).collect();
    let ideal_graded: HashMap<String, u8> = ideal_ids.iter().cloned()
...
fn compute_dcg(ranked: &[String], graded: &HashMap<String, u8>, k: usize) -> f64 {
    ranked.iter().take(k).enumerate().map(|(i, id)| {
