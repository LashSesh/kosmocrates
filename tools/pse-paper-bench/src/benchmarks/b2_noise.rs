...
pub fn run() -> B2Results {
    ...
    for rate in rates {
        ...
        let doc_texts: Vec<String> = ds.documents.iter().map(|d| {
        ...
        for (i, doc) in ds.documents.iter().enumerate() {
            let crystal = fixtures::build_crystal(&format!("b2-{rate:.2}-{}", doc.id), doc.stability, doc.kuramoto);
        ...
        for query in &ds.queries {
            ...
            let bm_ids: Vec<String> = bm_ranked.iter().map(|(i, _)| doc_id_map.get(&ds.documents[*i].id).cloned().unwrap_or_default()).collect();
        ...
        if rate == 0.0 { baseline_hit3 = pse_mean; }
        levels.push(NoiseLevel { rate, pse_hit3: pse_mean, bm25_hit3: bm_mean,
            pse_degradation: if rate == 0.0 { 0.0 } else { baseline_hit3 - pse_mean },
            bm25_degradation: 0.0 });
    }
    // Back-fill bm25 baseline
    let bm25_base = levels.first().map(|l| l.bm25_hit3).unwrap_or(0.0);
    for l in &mut levels { l.bm25_degradation = bm25_base - l.bm25_hit3; }
    B2Results { levels, baseline_hit3 }
}
