...
pub fn run() -> B7Results {
    ...
    for &n in SIZES {
        ...
        for i in 0..n {
            ...
            let _ = fixtures::commit_crystal(&mut store, &crystal,
                &format!("synthetic document {i} for scalability benchmark"), i + 1);
        }
        ...
        points.push(LatencyPoint { n_crystals: n, mean_us: mean, p95_us: p95, p99_us: p99 });
    }
    B7Results { points, n_retrieval_calls_per_point: N_CALLS }
}
