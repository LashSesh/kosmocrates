use crate::{datasets, baselines::keyword_filter, metrics::classification::{PrfMetrics, macro_f1}};
use pse_constitutional_interceptor::{ConstitutionalEvaluator, ActionContext, Decision};
use pse_nxalien_types::{RuleAtom, Severity, ScopeRef};
use serde::Serialize;
...
pub fn run() -> B3Results {
    ...
    let rules: Vec<RuleAtom> = ds.rules.iter().map(|r| {
    ...
    for action in &ds.actions {
        ...
        let kw_pred = if kw == keyword_filter::KeywordDecision::Block { "Block" } else { "Allow" };
        let kw_exp = if expected == "Block" { "Block" } else { "Allow" };
        if kw_pred == kw_exp {
            if kw_pred == "Block" { kw_tp[1] += 1; } else { kw_tp[0] += 1; }
    ...
    B3Results {
        pse_macro_f1: pse_macro, pse_allow, pse_warn, pse_block,
        keyword_macro_f1: kw_macro,
        n_actions: ds.actions.len(), n_rules: ds.rules.len(),
    }
}

fn class_idx(label: &str) -> Option<usize> {
    match label { "Allow" => Some(0), "Warn" => Some(1), "Block" => Some(2), _ => None }
}

fn update_counts(pred: &str, expected: &str,
    tp: &mut [usize; 3], fp: &mut [usize; 3], fn_: &mut [usize; 3]) {
    if pred == expected {
        if let Some(i) = class_idx(pred) { tp[i] += 1; }
    } else {
        if let Some(i) = class_idx(pred) { fp[i] += 1; }
        if let Some(i) = class_idx(expected) { fn_[i] += 1; }
    }
}
