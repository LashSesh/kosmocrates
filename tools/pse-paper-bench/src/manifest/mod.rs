...
pub fn build(out_dir: &Path, _results: &crate::benchmarks::AllResults) -> RunManifest {
    let files = ["b1_retrieval.json","b2_noise.json","b3_constitutional.json",
                 "b4_etv.json","b5_anomaly.json","b6_agent.json","b7_scalability.json",
                 "report.md","tables.tex"];
    let artifacts: Vec<ArtifactEntry> = files.iter().filter_map(|name| {
        let path = out_dir.join(name);
        let data = std::fs::read(&path).ok()?;
        let size_bytes = data.len() as u64;
        let sha256 = sha256_hex(&data);
        Some(ArtifactEntry { name: name.to_string(), sha256, size_bytes })
    }).collect();
