...
impl Bm25Index {
    pub fn build(raw_docs: &[String]) -> Self {
        ...
        let n = docs.len();
        let avgdl = if n == 0 { 1.0 }
                    else { docs.iter().map(|d| d.len() as f64).sum::<f64>() / n as f64 };
        let mut df: HashMap<String, usize> = HashMap::new();
        for doc in &docs {
            let terms: HashSet<&String> = doc.iter().collect();
            for t in terms { *df.entry(t.clone()).or_default() += 1; }
        }
        let idf = df.into_iter().map(|(term, df_t)| {
    ...
    pub fn score(&self, query: &str) -> Vec<f64> {
        ...
        self.docs.iter().map(|doc| {
            ...
            for t in doc { *tf_map.entry(t).or_default() += 1; }
            qtokens.iter().map(|qt| {
