...
use kosmo_core::{
    ...
    AcquisitionTaint,
    // R4 - Cartography
    CartographyEntryKind,
    CartographyStoreCommit,
    CartographyStoreError,
...
use kosmo_pse_bridge::{
    build_promotion_feedback, validate_candidate, PromotionOutcome, PromotionRequestRecord,
    PseBridgeCandidate, PseBridgeCandidateKind, PseBridgePolicy, PseBridgeRateLimit,
};

use kosmo_kcube::{kcube_file_name, KcubeArtifact, KcubeExecutor};
use kosmo_operator::{standard_plan, OperationPlan, OperatorExecutor};
use kosmo_parseback::{diff_snapshots, CrateFingerprint, ParseBackExecutor, TopologySnapshot};
use kosmo_pipeline::{run_dry_pipeline, IntegrationRunOptions};
...
fn build_scenarios() -> Vec<ScenarioResult> {
    ...
    v.push(run_check(
        "r1-sandbox-defaults-no-network",
        "R1:Foundry",
        || {
    ...
    v.push(run_check(
        "r1-cargo-policy-shell-disabled",
        "R1:Foundry",
        || {
    ...
    v.push(run_check(
        "r1-env-policy-locked-no-passthrough",
        "R1:Foundry",
        || {
            let env = FoundryEnvironmentPolicy::locked();
    ...
    v.push(run_check(
        "r2-severity-worst-wins-ordering",
        "R2:ParseBack",
        || {
    ...
    v.push(run_check(
        ...
        || {
            ...
            if d1.id != d2.id {
                return Err(format!(
                    "non-deterministic delta ids: {:?} vs {:?}",
                    d1.id, d2.id
                ));
            }
            if !d1.verify_id() {
    ...
    v.push(run_check(
        "r2-report-topology-unchanged-id-deterministic",
        "R2:ParseBack",
        || {
            let make =
                || ParseBackReport::topology_unchanged(d(b"plan"), d(b"topo"), d(b"bundle"), 42);
            let r1 = make();
            let r2 = make();
            if r1.id != r2.id {
                return Err(format!(
                    "non-deterministic report ids: {:?} vs {:?}",
                    r1.id, r2.id
                ));
            }
            if !r1.verify_id() {
    ...
    v.push(run_check(
        "r2-has-critical-delta-detection",
        "R2:ParseBack",
        || {
    ...
    v.push(run_check(
        "r3-both-fail-is-failed-both",
        "R3:ValidationClosure",
        || {
    ...
    v.push(run_check(
        "r3-foundry-fail-only-is-failed-foundry",
        "R3:ValidationClosure",
        || {
            let status = determine_closure_status(
                &FoundryExecutionOutcome::Failed,
    ...
    v.push(run_check(
        "r3-report-only-yields-inconclusive",
        "R3:ValidationClosure",
        || {
            ...
            if s1 != ValidationClosureStatus::Inconclusive {
                return Err(format!(
                    "expected Inconclusive (foundry skipped), got {:?}",
                    s1
                ));
            }
            ...
            if s2 != ValidationClosureStatus::Inconclusive {
                return Err(format!(
                    "expected Inconclusive (parseback skipped), got {:?}",
                    s2
                ));
            }
            Ok(())
    ...
    v.push(run_check(
        "r3-closure-report-id-deterministic",
        "R3:ValidationClosure",
        || {
            let make = || {
    ...
    v.push(run_check(
        "r4-report-only-denies-all-appends",
        "R4:Cartography",
        || {
    ...
    v.push(run_check(
        "r4-sequence-monotonic-enforced",
        "R4:Cartography",
        || {
            let mut store = InMemoryCartographyStore::new(CorpusScope::LocalHostProject, d(b"pol"));
    ...
    v.push(run_check(
        "r4-successful-append-and-integrity",
        "R4:Cartography",
        || {
            let mut store = InMemoryCartographyStore::new(CorpusScope::LocalHostProject, d(b"pol"));
            let dry_run = PolicyProfile::dry_run();
            ...
            if !report.status.is_intact() {
                return Err(format!(
                    "expected Intact after one append, got {:?}",
                    report.status
                ));
            }
            if report.checked_count != 1 {
                return Err(format!(
                    "expected checked_count=1, got {}",
    ...
    v.push(run_check(
        "r5-worktree-spec-id-deterministic",
        "R5:Materialization",
        || {
            let make = || {
    ...
    v.push(run_check(
        "r6-export-policy-report-only-no-write",
        "R6:KCube",
        || {
    ...
    v.push(run_check(
        "r6-roundtrip-verified-equal-digests",
        "R6:KCube",
        || {
            let archive_digest = d(b"archive_bytes");
            let v_pass =
                KcubeRoundtripVerification::new(archive_digest, archive_digest, d(b"bundle"));
            ...
            let v_fail =
                KcubeRoundtripVerification::new(d(b"archive_bytes"), d(b"different"), d(b"bundle"));
    ...
    v.push(run_check(
        "r6-roundtrip-verification-id-deterministic",
        "R6:KCube",
        || {
            let make =
                || KcubeRoundtripVerification::new(d(b"expected"), d(b"observed"), d(b"bundle"));
            let v1 = make();
            let v2 = make();
            if v1.id != v2.id {
    ...
    v.push(run_check(
        "r7-report-only-blocks-all-submissions",
        "R7:PSEBridge",
        || {
    ...
    v.push(run_check(
        "r7-deny-all-policy-blocks-dryrun",
        "R7:PSEBridge",
        || {
            let policy = PseBridgePolicy::deny_all(d(b"pol"));
            let candidate = PseBridgeCandidate::new(
    ...
    v.push(run_check(
        "r7-zero-evidence-bundle-rejected",
        "R7:PSEBridge",
        || {
            ...
            let candidate = PseBridgeCandidate::new(
                ...
                Q16::ONE,
                d(b"run"),
                Digest::ZERO, // ZERO evidence_bundle_id
                d(b"pol"),
            );
            let profile = PolicyProfile::dry_run();
            match validate_candidate(&candidate, &policy, &profile) {
    ...
    v.push(run_check(
        ...
        || {
            ...
            if c1.id != c2.id {
                return Err(format!(
                    "non-deterministic candidate ids: {:?} vs {:?}",
                    c1.id, c2.id
                ));
            }
            if !c1.verify_id() {
    ...
    v.push(run_check(
        "r8-taint-cleared-requires-both-checks",
        "R8:Acquisition",
        || {
            // Both pass → Cleared
            ...
            if cleared.taint != AcquisitionTaint::Cleared {
                return Err(format!(
                    "both-pass must yield Cleared, got {:?}",
                    cleared.taint
                ));
            }
    ...
    v.push(run_check(
        ...
        || {
            ...
            if s1.id != s2.id {
                return Err(format!(
                    "non-deterministic source ids: {:?} vs {:?}",
                    s1.id, s2.id
                ));
            }
            if !s1.verify_id() {
    ...
    v.push(run_check(
        "r9-deterministic-report-ids-invariant007",
        "R9:Evaluation",
        || {
            ...
            if !r1.outcome.is_passed() {
                return Err(format!(
                    "always_pass harness produced non-pass outcome: {:?}",
                    r1.outcome
                ));
            }
            Ok(())
    ...
    v.push(run_check(
        "r9-report-only-skips-execution",
        "R9:Evaluation",
        || {
            let scenario = EvaluationScenario::new(
                "ro-skip-test",
    ...
    v.push(run_check(
        "r9-always-fail-harness-fails-strict",
        "R9:Evaluation",
        || {
            let scenario = EvaluationScenario::new(
                "fail-test",
            ...
            if !report.outcome.is_failure_class() {
                return Err(format!(
                    "expected failure class outcome, got {:?}",
                    report.outcome
                ));
            }
            Ok(())
    ...
    v.push(run_check(
        "r9-suite-worst-wins-aggregation",
        "R9:Evaluation",
        || {
            ...
            if suite.suite_outcome != EvaluationSuiteOutcome::SomeFailed {
                return Err(format!(
                    "worst-wins: expected SomeFailed, got {:?}",
                    suite.suite_outcome
                ));
            }
    ...
    v.push(run_check(
        "r9-suite-all-pass-is-all-passed",
        "R9:Evaluation",
        || {
            let scenario = EvaluationScenario::new(
                "all-pass",
    ...
    v.push(run_check(
        "r9-policy-id-content-addressed",
        "R9:Evaluation",
        || {
    ...
    v.push(run_check(
        "rx-foundry-report-only-spawns-nothing",
        "RX:FoundryExec",
        || {
    ...
    v.push(run_check(
        "rx-foundry-command-denied-not-executed",
        "RX:FoundryExec",
        || {
            ...
            let exec =
                kosmo_foundry::FoundryExecutor::new(".").with_program("definitely-not-cargo");
            ...
            if report.outcome != kosmo_core::FoundryExecutionOutcome::CommandDeniedByPolicy {
                return Err(format!(
                    "expected CommandDeniedByPolicy, got {:?}",
                    report.outcome
                ));
            }
            if !report.verify_id() {
    ...
    v.push(run_check(
        "rx-foundry-kind-mapping-read-only",
        "RX:FoundryExec",
        || {
            ...
            if m(&K::Build) != Some("check") {
                return Err("Build must map to check".into());
            }
            if m(&K::Test) != Some("test") {
                return Err("Test must map to test".into());
            }
            if m(&K::Lint) != Some("clippy") {
                return Err("Lint must map to clippy".into());
            }
            // No mapping exposes a mutating command.
            if m(&K::Security).is_some() {
                return Err("Security must not map to cargo".into());
            }
            if m(&K::Custom("x".into())).is_some() {
                return Err("Custom must not map".into());
            }
            Ok(())
        },
    ));
    ...
    v.push(run_check(
        "rx-store-operator-approved-persists-reloads",
        "RX:PersistStore",
        || {
            use kosmo_core::{
            ...
            {
                let mut store =
                    kosmo_store::JsonlCartographyStore::open(&path, scope.clone(), d(b"pol"))
                        .map_err(|e| format!("open failed: {e}"))?;
                for seq in 1..=2u64 {
                    let commit = CartographyStoreCommit::new(
                        scope.clone(),
                        seq,
                        d(format!("p{seq}").as_bytes()),
                        CartographyEntryKind::EvidenceSummary,
                        d(b"bundle"),
                        d(b"pol"),
                    );
                    store
                        .append(commit, &PolicyProfile::operator_approved())
            ...
            if !intact {
                return Err(format!(
                    "expected Intact/2, got {:?}/{}",
                    report.status, report.checked_count
                ));
            }
            Ok(())
        },
    ...
    v.push(run_check(
        "rx-store-crystal-record-persist-and-reload",
        "RX:CrystalStore",
        || {
            use kosmo_core::{
            ...
            {
                let mut store =
                    CrystalRecordStore::open(&path).map_err(|e| format!("open: {e}"))?;
                store
                    .append(r1.clone(), &policy)
                    .map_err(|e| format!("append r1: {e}"))?;
                store
                    .append(r2.clone(), &policy)
                    .map_err(|e| format!("append r2: {e}"))?;
                // Dedup: re-appending r1 must be a no-op.
                store
                    .append(r1, &policy)
                    .map_err(|e| format!("dedup append: {e}"))?;
                if store.len() != 2 {
            ...
            let reopened = CrystalRecordStore::open(&path).map_err(|e| format!("reopen: {e}"))?;
            if reopened.len() != 2 {
                return Err(format!(
                    "expected 2 records after reload, got {}",
                    reopened.len()
                ));
            }
    ...
    v.push(run_check(
        "rx-parseback-report-only-skips-scan",
        "RX:ParseBackExec",
        || {
    ...
    v.push(run_check(
        "rx-parseback-baseline-mismatch-inconclusive",
        "RX:ParseBackExec",
        || {
            use kosmo_core::{ParseBackPlan, ParseBackScanScope};
            let executor = ParseBackExecutor::new(PathBuf::from("/nonexistent"));
    ...
    v.push(run_check(
        "rx-parseback-diff-node-added-warning",
        "RX:ParseBackExec",
        || {
    ...
    v.push(run_check(
        "rx-parseback-diff-node-removed-critical",
        "RX:ParseBackExec",
        || {
            use kosmo_core::ParseBackScanScope;
            use std::collections::{BTreeMap, BTreeSet};
    ...
    v.push(run_check(
        "rx-parseback-snapshot-deterministic",
        "RX:ParseBackExec",
        || {
    ...
    v.push(run_check(
        "rx-parseback-unchanged-workspace-passes",
        "RX:ParseBackExec",
        || {
            use kosmo_core::{ParseBackPlan, ParseBackScanScope};
            let workspace_root = PathBuf::from(env!("CARGO_MANIFEST_DIR"))
            ...
            if !report.outcome.is_passed() {
                return Err(format!(
                    "unchanged workspace must pass, got {:?}",
                    report.outcome
                ));
            }
            if !report.verify_id() {
    ...
    v.push(run_check(
        "rx-operator-report-only-inconclusive",
        "RX:Operator",
        || {
            ...
            let report =
                executor.execute(&plan, &PolicyProfile::default_report_only(), d(b"bundle"));
            if !matches!(
                report.closure_report.final_validation_status,
                ValidationClosureStatus::Inconclusive
            ) {
                return Err(format!(
                    "ReportOnly must be Inconclusive, got {:?}",
                    report.closure_report.final_validation_status
                ));
            }
            if report.persisted {
    ...
    v.push(run_check(
        "rx-operator-report-is-content-addressed",
        "RX:Operator",
        || {
            use kosmo_core::ParseBackScanScope;
            let plan = standard_plan(
            ...
            if r1.report_id != r2.report_id {
                return Err(
                    "INVARIANT-007: identical inputs must produce identical report_id".into(),
                );
            }
            if r1.report_id == d(b"") {
    ...
    v.push(run_check(
        "rx-operator-approved-persists-closure",
        "RX:Operator",
        || {
            use kosmo_core::{
                FoundryCheckKind, FoundryCheckSpec, FoundryCommandPolicy, FoundryEnvironmentPolicy,
                FoundryExecutionPlan, FoundrySandboxKind, FoundrySandboxSpec, FoundryTimeoutPolicy,
                ParseBackScanScope, ValidationClosureStatus,
            };
            let workspace_root = PathBuf::from(env!("CARGO_MANIFEST_DIR"))
                .join("..")
                .join("..");
            ...
            let check_spec =
                FoundryCheckSpec::new(FoundryCheckKind::Build, "kosmo-parseback", true)
                    .with_args(vec!["-p".into(), "kosmo-parseback".into()]);
            let foundry_plan = FoundryExecutionPlan::new(
                d(b"pol"),
                d(b"widx"),
            ...
            let executor = OperatorExecutor::new(&workspace_root).with_store(&store_path);
    ...
    v.push(run_check(
        "rx-pipeline-swarm-and-delta-in-report",
        "RX:Pipeline",
        || {
    ...
    v.push(run_check(
        "rx-pipeline-empty-workspace-delta-clean",
        "RX:Pipeline",
        || {
    ...
    v.push(run_check(
        "rx-pipeline-policy-consistency-includes-swarm",
        "RX:Pipeline",
        || {
    ...
    v.push(run_check(
        "rx-pipeline-report-is-deterministic",
        "RX:Pipeline",
        || {
    ...
    v.push(run_check(
        "rx-pipeline-collapse-plan-is-planning-only",
        "RX:Pipeline",
        || {
            ...
            if r.collapse_plan.status != CollapsePlanStatus::PlanningOnly {
                return Err(format!(
                    "expected PlanningOnly, got {:?}",
                    r.collapse_plan.status
                ));
            }
            Ok(())
        },
    ...
    v.push(run_check(
        "rx-pipeline-collapse-plan-carries-policy-id",
        "RX:Pipeline",
        || {
    ...
    v.push(run_check(
        "rx-pipeline-collapse-plan-is-content-addressed",
        "RX:Pipeline",
        || {
    ...
    v.push(run_check(
        "rx-pipeline-cartography-persist-blocked-by-default-policy",
        "RX:Pipeline",
        || {
    ...
    v.push(run_check(
        "rx-pipeline-cartography-persist-succeeds-with-operator-approved",
        "RX:Pipeline",
        || {
            ...
            let store =
                JsonlCartographyStore::open(&path, CorpusScope::LocalHostProject, policy.id)
                    .map_err(|e| format!("reopen failed: {:?}", e))?;
            let manifest = store
                .read_manifest()
    ...
    v.push(run_check(
        "rx-pipeline-morphogenic-update-carries-policy-id",
        "RX:Pipeline",
        || {
            ...
            if !r.verify_policy_consistency() {
                return Err(
                    "verify_policy_consistency must include morphogenic_update.policy_id".into(),
                );
            }
            Ok(())
        },
    ...
    v.push(run_check(
        "rx-pipeline-no-surgery-when-disabled",
        "RX:Pipeline",
        || {
    ...
    v.push(run_check(
        "rx-pipeline-surgery-produces-options-with-metatron",
        "RX:Pipeline",
        || {
    ...
    v.push(run_check(
        "rx-pipeline-surgery-options-are-energy-ranked",
        "RX:Pipeline",
        || {
            ...
            for window in r1.surgery_options.windows(2) {
                if window[0].confidence_score < window[1].confidence_score {
                    return Err(
                        "surgery_options must be ranked by confidence_score descending".into(),
                    );
                }
            }
            Ok(())
    ...
    v.push(run_check(
        "rx-pipeline-void-priority-ranking-length-matches-voids",
        "RX:Pipeline",
        || {
    ...
    v.push(run_check(
        "rx-pipeline-void-priority-ranking-is-deterministic",
        "RX:Pipeline",
        || {
            ...
            let void_ids: std::collections::HashSet<_> = r1
                .hyphae_result
                .host_cube
                .void_map
                .voids
    ...
    v.push(run_check(
        "rx-pipeline-metatron-index-empty-without-metatron",
        "RX:Pipeline",
        || {
            use kosmo_pipeline::{run_dry_pipeline, IntegrationRunOptions};
            use kosmo_workbench::WorkspaceIndex;
            let policy = PolicyProfile::default_report_only();
            let entries = vec![kosmo_workbench::WorkspaceEntry {
                path: "src/lib.rs".into(),
                digest: Digest::of_bytes(b"lib"),
                size_bytes: 100,
            ...
            let r = run_dry_pipeline(&index, &IntegrationRunOptions::report_only(), &policy);
            if !r.metatron_index.micrograph_ids.is_empty()
                || !r.metatron_index.diagnostic_ids.is_empty()
            {
    ...
    v.push(run_check(
        "rx-pipeline-metatron-index-content-addressed",
        "RX:Pipeline",
        || {
            use kosmo_pipeline::{run_dry_pipeline, IntegrationRunOptions};
            use kosmo_workbench::WorkspaceIndex;
            ...
            if r1.metatron_index.micrograph_ids.len() != void_count {
                return Err(format!(
                    ...
                    void_count,
                    r1.metatron_index.micrograph_ids.len()
                ));
            }
    ...
    v.push(run_check(
        "rx-pipeline-lift-reports-empty-without-metatron",
        "RX:Pipeline",
        || {
            use kosmo_pipeline::{run_dry_pipeline, IntegrationRunOptions};
            use kosmo_workbench::WorkspaceIndex;
    ...
    v.push(run_check(
        "rx-pipeline-lift-reports-one-per-void-with-metatron",
        "RX:Pipeline",
        || {
            use kosmo_pipeline::{run_dry_pipeline, IntegrationRunOptions};
            use kosmo_workbench::WorkspaceIndex;
    ...
    v.push(run_check(
        "rx-pipeline-motif-candidates-disabled-by-default",
        "RX:Pipeline",
        || {
            use kosmo_pipeline::{run_dry_pipeline, IntegrationRunOptions};
            use kosmo_workbench::WorkspaceIndex;
    ...
    v.push(run_check(
        "rx-pipeline-motif-candidates-from-void-kind-frequency",
        "RX:Pipeline",
        || {
            use kosmo_pipeline::{run_dry_pipeline, IntegrationRunOptions};
            use kosmo_workbench::WorkspaceIndex;
            ...
            if r.hyphae_result.host_cube.void_count() > 0 {
                ...
                for c in &r.motif_candidates {
                    if !c.name.starts_with("motif:") {
                        return Err(format!(
                            "motif name must start with 'motif:', got '{}'",
                            c.name
                        ));
                    }
    ...
    v.push(run_check(
        "rx-pipeline-norm-candidates-disabled-by-default",
        "RX:Pipeline",
        || {
            use kosmo_pipeline::{run_dry_pipeline, IntegrationRunOptions};
            use kosmo_workbench::WorkspaceIndex;
    ...
    v.push(run_check(
        "rx-pipeline-norm-candidates-from-accepted-decisions",
        "RX:Pipeline",
        || {
            use kosmo_pipeline::{run_dry_pipeline, IntegrationRunOptions};
            use kosmo_workbench::WorkspaceIndex;
            ...
            for c in &r.norm_candidates {
                ...
                if c.evidence_bundle_id == Digest::ZERO {
                    return Err(
                        "CROSS-006: norm candidate evidence_bundle_id must be non-ZERO".into(),
                    );
                }
                if c.fitness_score != Q16::ONE {
                    return Err(format!(
                        "initial fitness must be Q16::ONE, got {:?}",
                        c.fitness_score
                    ));
                }
            }
    ...
    v.push(run_check(
        "rx-pipeline-surgery-workbench-tasks-disabled-by-default",
        "RX:Pipeline",
        || {
            use kosmo_pipeline::{run_dry_pipeline, IntegrationRunOptions};
            use kosmo_workbench::WorkspaceIndex;
    ...
    v.push(run_check(
        "rx-pipeline-surgery-workbench-tasks-match-surgery-options",
        "RX:Pipeline",
        || {
            use kosmo_pipeline::{run_dry_pipeline, IntegrationRunOptions};
            ...
            if !r.verify_policy_consistency() {
                return Err(
                    "verify_policy_consistency must include surgery_workbench_tasks".into(),
                );
            }
            // Determinism.
            let r2 = run_dry_pipeline(&index, &opts, &policy);
    ...
    v.push(run_check(
        "rx-pipeline-deficiency-vector-always-present",
        "RX:Pipeline",
        || {
            use kosmo_pipeline::{run_dry_pipeline, IntegrationRunOptions};
            use kosmo_workbench::WorkspaceIndex;
    ...
    v.push(run_check(
        "rx-pipeline-deficiency-vector-captures-void-kinds",
        "RX:Pipeline",
        || {
            use kosmo_pipeline::{run_dry_pipeline, IntegrationRunOptions};
            use kosmo_workbench::{WorkspaceEntry, WorkspaceEntryKind, WorkspaceIndex};
            ...
            {
                return Err(
                    "non-empty deficiency entries must yield non-zero total_severity".into(),
                );
            }
            Ok(())
        },
    ...
    v.push(run_check(
        "rx-pipeline-pse-candidates-disabled-by-default",
        "RX:Pipeline",
        || {
            use kosmo_pipeline::{run_dry_pipeline, IntegrationRunOptions};
            use kosmo_workbench::WorkspaceIndex;
    ...
    v.push(run_check(
        "rx-pipeline-pse-candidates-cover-norm-and-topology",
        "RX:Pipeline",
        || {
            use kosmo_pipeline::{run_dry_pipeline, IntegrationRunOptions};
            use kosmo_workbench::{WorkspaceEntry, WorkspaceEntryKind, WorkspaceIndex};
            ...
            for c in &r.pse_candidates {
                ...
                if c.evidence_bundle_id == Digest::ZERO {
                    return Err(
                        "CROSS-006: PSE candidate evidence_bundle_id must be non-ZERO".into(),
                    );
                }
            }
            if !r.verify_policy_consistency() {
    ...
    v.push(run_check(
        "rx-pipeline-pse-candidates-include-motif-observations",
        "RX:Pipeline",
        || {
    ...
    v.push(run_check(
        "rx-pipeline-crystal-candidates-disabled-by-default",
        "RX:Pipeline",
        || {
            use kosmo_pipeline::{run_dry_pipeline, IntegrationRunOptions};
            use kosmo_workbench::WorkspaceIndex;
    ...
    v.push(run_check(
        "rx-pipeline-crystal-candidates-one-per-accepted-decision",
        "RX:Pipeline",
        || {
            use kosmo_pipeline::{run_dry_pipeline, IntegrationRunOptions};
            use kosmo_workbench::{WorkspaceEntry, WorkspaceEntryKind, WorkspaceIndex};
            ...
            for c in &r.crystal_candidates {
                ...
                if c.support_score != Q16::ZERO {
                    return Err(
                        "support_score must be ZERO at creation (Pending certification)".into(),
                    );
                }
            }
            if !r.verify_policy_consistency() {
    ...
    v.push(run_check(
        "rx-pipeline-norm-fitness-traces-empty-without-feedback",
        "RX:Pipeline",
        || {
            use kosmo_pipeline::{run_dry_pipeline, IntegrationRunOptions};
            use kosmo_workbench::WorkspaceIndex;
    ...
    v.push(run_check(
        "rx-pipeline-norm-fitness-traces-from-matching-feedback",
        "RX:Pipeline",
        || {
            use kosmo_pipeline::{run_dry_pipeline, IntegrationRunOptions};
            use kosmo_workbench::{WorkspaceEntry, WorkspaceEntryKind, WorkspaceIndex};
            ...
            if r_fb.report_id != r_fb2.report_id {
                return Err(
                    "norm_fitness_traces must participate in report_id deterministically".into(),
                );
            }
            Ok(())
        },
    ...
    v.push(run_check(
        "rx-pipeline-systemcube-tainted-units-warn-not-reject",
        "RX:Pipeline",
        || {
            let policy = PolicyProfile::default_report_only();
            let index = WorkspaceIndex::from_entries("test".into(), vec![], policy.id);
            ...
            let r = run_dry_pipeline(&index, &opts, &policy);
            let score = r
                .systemcube_compatibility_score()
            ...
            if score >= Q16::ONE {
                return Err(format!(
                    "expected score < ONE for tainted units, got {}",
                    score.raw()
                ));
            }
    ...
    v.push(run_check(
        "rx-pipeline-systemcube-summary-includes-diagnostics",
        "RX:Pipeline",
        || {
            let policy = PolicyProfile::default_report_only();
            let index = WorkspaceIndex::from_entries("test".into(), vec![], policy.id);
            ...
            if !s.contains("compat=") {
                return Err(format!(
                    "summary must include compat= field, got: {}",
                    &s[..s.len().min(200)]
                ));
            }
    ...
    v.push(run_check(
        "rx-pipeline-decision-taint-propagates-to-blueprint-units",
        "RX:Pipeline",
        || {
            ...
            for gap in &export.compatibility.gaps {
                if gap.gap_kind != "TaintedUnit" {
                    return Err(format!(
                        "unexpected gap kind '{}'; expected TaintedUnit from Synthetic decisions",
                        gap.gap_kind
                    ));
                }
            }
            ...
            if r.report_id != r2.report_id {
                return Err(
                    "taint-carrying decisions must be deterministic (INVARIANT-007)".into(),
                );
            }
            Ok(())
        },
    ...
    v.push(run_check(
        "rx-hyphae-clean-intent-yields-accepted-decision",
        "RX:Hyphae",
        || {
    ...
    v.push(run_check(
        "rx-hyphae-reduce-deficiency-intent-carries-kind-ref",
        "RX:Hyphae",
        || {
    ...
    v.push(run_check(
        "rx-hyphae-frontier-includes-reduce-deficiency-intents",
        "RX:Hyphae",
        || {
            ...
            if reduce_count != 2 {
                return Err(format!(
                    "expected 2 ReduceDeficiency intents (TestCoverage + Documentation), got {}",
                    reduce_count
                ));
            }
    ...
    v.push(run_check(
        "rx-hyphae-suggest-pattern-intent-produces-motif-proposal-yield",
        "RX:Hyphae",
        || {
    ...
    v.push(run_check(
        "rx-pipeline-prior-motifs-inject-suggest-pattern-intents",
        "RX:Pipeline",
        || {
            ...
            if suggest_count == 0 {
                return Err(
                    "prior_motifs must inject SuggestPattern intents into the frontier".into(),
                );
            }
    ...
    v.push(run_check(
        "rx-hyphae-assimilation-ledger-content-addressed",
        "RX:Hyphae",
        || {
            ...
            let l1 =
                AssimilationLedger::from_decisions(std::slice::from_ref(&d1), run_id, policy.id);
            let l2 =
                AssimilationLedger::from_decisions(std::slice::from_ref(&d1), run_id, policy.id);
            ...
            let l3 =
                AssimilationLedger::from_decisions(std::slice::from_ref(&d2), run_id, policy.id);
    ...
    v.push(run_check(
        "rx-crystal-certify-pending-candidate-produces-record",
        "RX:Crystal",
        || {
    ...
    v.push(run_check(
        "rx-pipeline-certified-crystals-from-candidates",
        "RX:Pipeline",
        || {
    ...
    v.push(run_check(
        "rx-hyphae-hdag-extracted-from-source-content",
        "RX:Hyphae",
        || {
            ...
            let no_content_entries = vec![WorkspaceEntry {
                path: "src/lib.rs".into(),
                digest: Digest::of_bytes(b"lib"),
                size_bytes: 100,
                kind: WorkspaceEntryKind::SourceFile,
            ...
            let no_content_index =
                WorkspaceIndex::from_entries("test-root".into(), no_content_entries, policy.id);
    ...
    v.push(run_check(
        "rx-hyphae-hdag-severity-scales-with-definition-count",
        "RX:Hyphae",
        || {
            use kosmo_core::{Digest, PolicyProfile};
            use kosmo_hyphae::host::HostCube;
    ...
    v.push(run_check(
        ...
        || {
            ...
            if record.source_void_id != Some(void_id) {
                return Err(format!(
                    "source_void_id mismatch: {:?}",
                    record.source_void_id
                ));
            }
            if record.rho_coherence != rho {
                return Err(format!(
                    "rho_coherence mismatch: got {}",
                    record.rho_coherence.raw()
                ));
            }
            if record.omega_phase != omega {
                return Err(format!(
                    "omega_phase mismatch: got {}",
                    record.omega_phase.raw()
                ));
            }
            if record.record_id == Digest::ZERO {
                return Err("record_id must be non-ZERO".into());
    ...
    v.push(run_check(
        "rx-crystal-resonite-from-records-is-symmetric-and-bounded",
        "RX:Crystal",
        || {
            use kosmo_core::{
                AuthorityLabel, Digest, EvidenceBundle, EvidenceKind, EvidenceRef, PolicyProfile,
            ...
            let r2 = make_record(Q16::ratio(1, 4).unwrap(), Q16::ratio(3, 4).unwrap(), b"b")
                .ok_or("certify failed for r2")?;
            ...
            if res_ab.resonance_score.raw() < 0 || res_ab.resonance_score.raw() > Q16::ONE.raw() {
                return Err(format!(
                    "resonance_score out of bounds: {}",
                    res_ab.resonance_score.raw()
                ));
            }
            ...
            if res_same.resonance_score != Q16::ONE {
                return Err(format!(
                    "identical records must score ONE, got {}",
                    res_same.resonance_score.raw()
                ));
            }
    ...
    v.push(run_check(
        "rx-pipeline-resonite-map-populated-with-prior-crystals",
        "RX:Pipeline",
        || {
            use kosmo_core::{Digest, PolicyProfile};
            use kosmo_pipeline::{run_dry_pipeline, IntegrationRunOptions};
    ...
    v.push(run_check(
        "rx-pipeline-crystal-resonance-boost-absent-without-prior",
        "RX:Pipeline",
        || {
            use kosmo_core::{Digest, PolicyProfile};
            use kosmo_pipeline::{run_dry_pipeline, IntegrationRunOptions};
    ...
    v.push(run_check(
        "rx-pipeline-crystal-auto-persist-report-only-no-write",
        "RX:CrystalPersist",
        || {
            use kosmo_core::{Digest, PolicyProfile};
            use kosmo_pipeline::{run_dry_pipeline, IntegrationRunOptions};
    ...
    v.push(run_check(
        "rx-pipeline-crystal-auto-persist-persisted-count-is-observational",
        "RX:CrystalPersist",
        || {
            ...
            if r_yes.persisted_crystal_count > r_yes.certified_crystals.len() as u32 {
                return Err(format!(
                    ...
                    r_yes.persisted_crystal_count,
                    r_yes.certified_crystals.len()
                ));
            }
            Ok(())
        },
    ...
    v.push(run_check(
        "rx-pipeline-run-workspace-pipeline-on-temp-dir",
        "RX:WorkspacePipeline",
        || {
    ...
    v.push(run_check(
        "rx-pipeline-workspace-session-accumulates-across-runs",
        "RX:WorkspacePipeline",
        || {
            use kosmo_core::PolicyProfile;
    ...
    v.push(run_check(
        "rx-pipeline-action-items-sorted-descending",
        "RX:ActionItem",
        || {
            use kosmo_core::{Digest, PolicyProfile};
            use kosmo_pipeline::{run_dry_pipeline, ActionItemKind, IntegrationRunOptions};
            use kosmo_workbench::{WorkspaceEntry, WorkspaceEntryKind, WorkspaceIndex};
    ...
    v.push(run_check(
        "rx-pipeline-action-items-top-void-scores-one",
        "RX:ActionItem",
        || {
    ...
    v.push(run_check(
        "rx-blueprint-energy-accepted-positive-opaque-zero",
        "RX:BlueprintEnergy",
        || {
            let policy = PolicyProfile::default_report_only();
            ...
            if eo.energy.raw() != 0 {
                return Err(format!(
                    "opaque-rejected BlueprintUnit must have zero energy, got {}",
                    eo.energy.raw()
                ));
            }
            ...
            if ea.evidence_bundle_id != accepted.unit_id {
                return Err(
                    "evidence_bundle_id must equal unit_id (self-referential, CROSS-006)".into(),
                );
            }
            Ok(())
        },
    ...
    v.push(run_check(
        "rx-blueprint-energy-tainted-ranks-below-clean",
        "RX:BlueprintEnergy",
        || {
            let policy = PolicyProfile::default_report_only();
            let ev = Digest::of_bytes(b"ev");
    ...
    v.push(run_check(
        "rx-energy-tripolar-is-exact-product",
        "RX:Energy",
        || {
    ...
    v.push(run_check(
        "rx-energy-reject-gate-never-bypassed",
        "RX:Energy",
        || {
    ...
    v.push(run_check(
        "rx-energy-quarantine-and-proprietary-zero",
        "RX:Energy",
        || {
            ...
            for factors in [
                ...
                if !k.is_zeroed() {
                    return Err(
                        "quarantine / proprietary / foundry-failure must zero energy".into(),
                    );
                }
            }
            Ok(())
    ...
    v.push(run_check(
        "rx-energy-assessment-content-addressed",
        "RX:Energy",
        || {
    ...
    v.push(run_check(
        "rx-energy-ranking-deterministic",
        "RX:Energy",
        || {
    ...
    v.push(run_check(
        "rx-energy-source-cube-quarantine-zeroes-energy",
        "RX:EnergyRanking",
        || {
    ...
    v.push(run_check(
        "rx-energy-source-cube-ranking-picks-best",
        "RX:EnergyRanking",
        || {
            ...
            if fill.action
                != (kosmo_hyphae::delta::DeltaAction::FillVoid {
                    top_candidate_cube_id: hi.cube_id,
                })
            {
                return Err(format!(
                    "wrong top candidate: expected hi cube, got {:?}",
                    fill.action
                ));
            }
    ...
    v.push(run_check(
        "rx-energy-norm-candidate-assessment-content-addressed",
        "RX:EnergyRanking",
        || {
            ...
            let ev_id = d(b"e");
            let cand =
                NormGeneCandidate::new("test-norm".into(), "desc".into(), Q16::HALF, ev_id, pid);
            ...
            let rejected = cand.energy_assessment(&GateResult::Reject {
                reason: "test".into(),
            });
    ...
    v.push(run_check(
        "rx-energy-motif-assessment-content-addressed",
        "RX:EnergyRanking",
        || {
    ...
    v.push(run_check(
        ...
        || {
            ...
            if !a_bad.kernel.is_zeroed() || !a_good.kernel.is_zeroed() {
                return Err(
                    "seam=0 must collapse energy to zero for all cubes targeting that void".into(),
                );
            }
    ...
    v.push(run_check(
        "rx-energy-crystal-candidate-content-addressed",
        "RX:EnergyRanking",
        || {
            ...
            use kosmo_hyphae::{
                AssimilationDecision, GateCascade, StructuralCrystalCandidate, StructuralYield,
                StructuralYieldKind,
            };
            let policy = PolicyProfile::default_report_only();
    ...
    v.push(run_check(
        "rx-energy-crystal-candidate-reject-gate-zeroes-energy",
        "RX:EnergyRanking",
        || {
            ...
            let reject_a = candidate.energy_assessment(&GateResult::Reject {
                reason: "test".into(),
            });
            ...
            if !pass_a.kernel.is_zeroed() {
                return Err(
                    "zero support_score must yield zero energy (CROSS-007 invariant)".into(),
                );
            }
            Ok(())
        },
...
";
    ...
    v.push(run_check(
        "rx-topology-extracts-real-graph",
        "RX:Topology",
        || {
    ...
    v.push(run_check(
        "rx-topology-extraction-deterministic",
        "RX:Topology",
        || {
            let a = CodeHDAG::extract_from_rust_source(
                d(b"ev"),
                "x.rs",
                TOPO_SAMPLE,
                TaintLabel::Clean,
            );
            let b = CodeHDAG::extract_from_rust_source(
                d(b"ev"),
                "x.rs",
                TOPO_SAMPLE,
                TaintLabel::Clean,
    ...
    v.push(run_check("rx-topology-energy-chain", "RX:Topology", || {
        ...
        let empty_e = empty
            .energy_kernel(psi, EnergyFactors::all_clean())
            .energy();
    ...
    v.push(run_check(
        "rx-kcube-denied-when-write-false",
        "RX:Kcube",
        || {
    ...
    v.push(run_check(
        "rx-kcube-write-and-roundtrip-passes",
        "RX:Kcube",
        || {
    ...
    v.push(run_check(
        "rx-kcube-package-content-addressed",
        "RX:Kcube",
        || {
            ...
            let pkg = exec.read(&fname).map_err(|e| format!("read failed: {e}"))?;
            if !pkg.verify_id() {
    ...
    v.push(run_check(
        "rx-kcube-overwrite-denied-by-default",
        "RX:Kcube",
        || {
            ...
            if r2.outcome.is_written() {
                return Err(
                    "second write to same sequence must be blocked (allow_overwrite=false)".into(),
                );
            }
            if r2.outcome.is_failure_class()
                && !matches!(
                    r2.outcome,
                    kosmo_core::KcubeWriteOutcome::DeniedByPolicy { .. }
                )
            {
    ...
    v.push(run_check(
        "rx-systemcube-kcube-blocked-default-policy",
        "RX:SystemCubeKcube",
        || {
            ...
            let cube = SystemCube::new(
                d(b"host"),
                &kosmo_core::RunDescriptor::new(policy.id, "host"),
                &policy,
                vec![unit],
            );
    ...
    v.push(run_check(
        "rx-systemcube-kcube-writes-archive",
        "RX:SystemCubeKcube",
        || {
            ...
            let report =
                cube.export_to_kcube(&exec, 4, &export_policy, &op_policy, d(b"bundle"), 1);
            if !report.outcome.is_written() {
                return Err(format!("expected Written, got {:?}", report.outcome));
            }
            if !report.roundtrip_passed() {
    ...
    v.push(run_check(
        "rx-systemcube-kcube-archive-parses-back",
        "RX:SystemCubeKcube",
        || {
            ...
            let report =
                cube.export_to_kcube(&exec, 4, &export_policy, &op_policy, d(b"bundle"), 42);
            if !report.outcome.is_written() {
                return Err(format!("write failed: {:?}", report.outcome));
            }
            ...
            if pkg.entry_count() != 4 {
                return Err(format!(
                    "expected 4 artifacts (manifest + report + 2 units), got {}",
                    pkg.entry_count()
                ));
            }
            // package_id in the write report must match the parsed package id
    ...
    v.push(run_check(
        "rx-contradiction-energy-role-conflict-detected",
        "RX:ContradictionEnergy",
        || {
            let policy = PolicyProfile::default_report_only();
            ...
            if r.contradictions[0].weight != Q16::ONE {
                return Err(format!(
                    "RoleConflict weight must be Q16::ONE, got {}",
                    r.contradictions[0].weight.raw()
                ));
            }
    ...
    v.push(run_check(
        "rx-contradiction-energy-independent-sources-zero",
        "RX:ContradictionEnergy",
        || {
            let policy = PolicyProfile::default_report_only();
            let mid = d(b"manifest");
            ...
            if r.total_energy != Q16::ZERO {
                return Err(format!(
                    "independent sources must yield zero energy, got {}",
                    r.total_energy.raw()
                ));
            }
            if !r.contradictions.is_empty() {
                return Err(format!(
                    "expected 0 contradictions, got {}",
                    r.contradictions.len()
                ));
            }
            ...
            if r.report_id != r2.report_id {
                return Err(
                    "ContradictionEnergyReport must be deterministic (INVARIANT-007)".into(),
                );
            }
            Ok(())
        },
    ...
    v.push(run_check(
        "rx-compatibility-clean-units-score-one",
        "RX:Compatibility",
        || {
            let policy = PolicyProfile::default_report_only();
            let mid = d(b"manifest");
            ...
            let r = CompatibilityProfileReport::from_units(
                mid,
                hid,
                &policy,
                &[u1.clone(), u2.clone()],
            ...
            if r.compatibility_score != Q16::ONE {
                return Err(format!(
                    "all-clean units must yield score=ONE, got {}",
                    r.compatibility_score.raw()
                ));
            }
            if !r.gaps.is_empty() {
            ...
            if r.report_id != r2.report_id {
                return Err(
                    "CompatibilityProfileReport must be deterministic (INVARIANT-007)".into(),
                );
            }
            Ok(())
        },
    ...
    v.push(run_check(
        "rx-compatibility-tainted-unit-reduces-score",
        "RX:Compatibility",
        || {
            let policy = PolicyProfile::default_report_only();
            let mid = d(b"manifest");
            ...
            if r.gaps[0].gap_kind != "TaintedUnit" {
                return Err(format!(
                    "expected TaintedUnit gap, got {}",
                    r.gaps[0].gap_kind
                ));
            }
            if r.gaps[0].severity != Q16::HALF {
                return Err(format!(
                    "TaintedUnit severity must be HALF, got {}",
                    r.gaps[0].severity.raw()
                ));
            }
            if r.compatibility_score != Q16::HALF {
                return Err(format!(
                    "one tainted unit must yield score=HALF, got {}",
                    r.compatibility_score.raw()
                ));
            }
            Ok(())
    ...
    v.push(run_check(
        "rx-feedback-accepted-maps-to-full-energy",
        "RX:FeedbackLoop",
        || {
            ...
            if f.fitness_signal != energy {
                return Err(format!(
                    "accepted must map fitness to energy, got {:?}",
                    f.fitness_signal.raw()
                ));
            }
            if !f.verify_id() {
    ...
    v.push(run_check(
        "rx-feedback-stored-in-cartography",
        "RX:FeedbackLoop",
        || {
            ...
            if !commit.verify_id() {
                return Err(
                    "CartographyStoreCommit with PromotionFeedback must verify_id()".into(),
                );
            }
    ...
    v.push(run_check(
        "rx-feedback-norm-trace-updates-from-feedback",
        "RX:FeedbackLoop",
        || {
            ...
            let record =
                PromotionRequestRecord::new(candidate.id, PromotionOutcome::Accepted, d(b"ev"), 10);
            ...
            if feedback.fitness_signal != energy {
                return Err(format!(
                    "fitness_signal must equal energy={}, got {}",
                    energy.raw(),
                    feedback.fitness_signal.raw()
                ));
            }
            ...
            let trace =
                NormFitnessTrace::empty(norm_cid, policy.id).observe_from_feedback(&feedback);
            if trace.latest_fitness() != energy {
                return Err(
                    "NormFitnessTrace.latest_fitness must equal energy after feedback ingestion"
                        .to_string(),
                );
            }
            if trace.observations[0].evidence_ref != feedback.id {
...
fn run_cerebras(api_key: &str) -> ScenarioResult {
    ...
    let model = std::env::var("CEREBRAS_MODEL").unwrap_or_else(|_| "gpt-oss-120b".to_string());
