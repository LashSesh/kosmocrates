...
### The wish becomes a rendered hypercube — the Wunschkalkül arc, Runs 3–11 (2026-06-15)
...
- **Run 11 — the honesty verdict, graded by depth**: Run 9's over-fit signal was too blunt — it branded a trivial *existence* wish over a small workspace a "hologram" (a false alarm the Run 10 smoke exposed). Now the verdict is graded by the deepest stratum the wish actually claims (`cube.solid_frontier()`): the *earned* strata (Verified/Live, rank ≥ 3) must be backed by exercised structure, so a solid claim there over a sparse topology is the dangerous hologram — `⚠ over-fit at depth — a {live} claim stands over a sparse topology … a hologram, not a diamond`. A solid *declarative* stratum (existence/shape/wiring) over the same sparse topology is honest — `· thin but shallow — only {existence} was claimed; a sparse topology is honest for so shallow a wish` (a gentle note, not an alarm). Dense topology reads `genuine` at any depth. Still advisory (CROSS-010), still render-only; this sharpens the very signal Run 9 introduced so it cries hologram only when a behavioral/runtime claim lacks the mass to back it — the over-fit the source specs warn against.
