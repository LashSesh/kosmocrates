    ...
    F: FnOnce() -> Vec<Vec<u8>>,
{
    ...
    println!(
        "{} — processing {} observations ({} per tick)...",
...
fn cmd_status() {
    ...
    println!(
        "  Memory:      ~{} bytes (graph)",
        state.graph.estimate_heap_size()
    );
}
...
fn cmd_observe_entsoe(args: &[String]) {
    ...
    let observations = if let Some(path) = csv_path.filter(|_| !embedded) {
        match pse_adapter_entsoe::load_csv(std::path::Path::new(&path)) {
            Ok(obs) => obs,
            Err(e) => {
                eprintln!("CSV error: {}. Using embedded.", e);
...
fn cmd_observe_seismo(_args: &[String]) {
    ...
    run_observe(
        &adapter,
        || {
            pse_adapter_seismo::embedded_seismo_data()
                .iter()
                .map(|e| serde_json::to_vec(e).unwrap())
                .collect()
        },
        "USGS Seismology (embedded)",
    );
}
...
fn cmd_observe_weather(_args: &[String]) {
    ...
    run_observe(
        &adapter,
        || {
            pse_adapter_weather::embedded_weather_data()
                .iter()
                .map(|r| serde_json::to_vec(r).unwrap())
                .collect()
        },
        "Open-Meteo Weather (embedded)",
    );
}
...
fn cmd_observe_airquality(_args: &[String]) {
    ...
    run_observe(
        &adapter,
        || {
            pse_adapter_airquality::embedded_airquality_data()
                .iter()
                .map(|r| serde_json::to_vec(r).unwrap())
                .collect()
        },
        "OpenAQ Air Quality (embedded)",
    );
}
...
fn cmd_quality(args: &[String]) {
    ...
    for stat in &stats {
        println!(
            "  {}: min={:.2?} max={:.2?} mean={:.2?} nulls={}",
            stat.name, stat.min, stat.max, stat.mean, stat.null_count
        );
    }
    ...
    if !anomalies.is_empty() {
        println!("\nOutliers: {}", anomalies.len());
        for a in &anomalies {
            println!("  {}", a.description);
        }
    }
    if !drifts.is_empty() {
        println!("\nDrift events: {}", drifts.len());
        for d in &drifts {
    ...
    let mut cc = 0;
    for row in &rows {
        let b = vec![serde_json::to_vec(row).unwrap()];
        if let Ok(Some(_)) = macro_step(&mut state, &b, &config, &adapter) {
            cc += 1;
        }
    }
    println!("\nPSE crystals: {}", cc);
...
fn cmd_crystals(args: &[String]) {
    let json_format =
        args.iter().any(|a| a == "json") && args.iter().any(|a| a == "--format" || a == "json");
    let config = Config::default();
    let mut state = GlobalState::new(&config);
    ...
    if json_format {
        println!(
            "{}",
            serde_json::to_string_pretty(crystals).unwrap_or_default()
        );
    } else {
...
fn cmd_audit(args: &[String]) {
    ...
    if verify_only {
        println!(
            "Integrity: chains={} hashes={}",
            report.integrity_check.all_chains_valid, report.integrity_check.all_hashes_match
        );
        return;
    }
...
fn cmd_memory() {
    use pse_store::{CrystalStore, IslandStore};
    ...
    match IslandStore::open(db_path) {
        Ok(store) => {
            ...
            if let Ok(rows) = store.list_all_crystals() {
                let crystals: Vec<pse_types::SemanticCrystal> = rows
                    .iter()
            ...
            println!(
                "  Index size: {:.1} KB",
                stats.index_size as f64
                    * std::mem::size_of::<pse_memory::CrystalSignature>() as f64
                    / 1024.0
            );
        }
...
fn cmd_build_wasm() {
    ...
    match status {
        ...
        Err(e) => eprintln!(
            "Failed to run wasm-pack: {}. Install with: cargo install wasm-pack",
            e
        ),
    }
}
...
fn main() {
    ...
    match args[1].as_str() {
        ...
        "status" => cmd_status(),
        "run" => {
            let t = args.get(2).and_then(|s| s.parse().ok()).unwrap_or(10);
            cmd_run(t);
        }
        "observe" => {
            if args.len() < 3 {
                eprintln!("Usage: pse observe <adapter>");
                std::process::exit(1);
            }
            match args[2].as_str() {
        ...
        "serve" => {
            ...
            let state =
                std::sync::Arc::new(tokio::sync::RwLock::new(pse_gateway::AppState::default()));
            rt.block_on(async {
